...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        # return self.make(shape=new_shape, handle=self._handle, device=self.device)
    ...
    def permute(self, new_axes):
        ...
        if new_axes == tuple(range(dims)):
            return self
        new_shape = tuple(numpy.array(self.shape)[list(new_axes)])
        new_stride = tuple(numpy.array(self.strides)[list(new_axes)])
        
        return self.as_strided(shape=new_shape, strides=new_stride)
        # return self.make(shape=new_shape, strides=new_stride, handle=self._handle, device=self.device)
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(self.shape)):
            if (self.shape[i] != 1 and new_shape[i] != self.shape[i]):
                raise RuntimeError
        new_stride = numpy.zeros(len(new_shape), dtype=int)
        for i in range(len(new_shape)):
          if (new_shape[i] == self.shape[i]):
            new_stride[i] = self.strides[i]
        new_stride = tuple(new_stride)
        # return self.make(new_shape, strides=new_stride, handle=self._handle, device=self.device)
