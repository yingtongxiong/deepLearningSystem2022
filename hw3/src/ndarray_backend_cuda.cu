...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size)
  {
    ...
    {
      offset += gid_c % shape.data[i] * strides.data[i];
      gid_c /= shape.data[i];
    }
    out[gid] = a[offset];
  }
...
__global__ void MatmulKernel_simp(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P) {
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 grid = dim3(gird_x, grid_y, 1);
  // MatmulKernel_simp<<<grid, block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  ...
  if (gid < out_size) {
    ...
    for (size_t i = 1; i < reduce_size; ++i) {
      out[gid] = out[gid] > a[start + i] ? out[gid] : a[start + i];
    }
  }
}
