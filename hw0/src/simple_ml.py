...
def softmax_loss(Z, y):
    ...
    ### BEGIN YOUR CODE
    tmp = np.exp(Z)
    index1 = list(np.arange(tmp.shape[0]))
    res = np.log(np.sum(tmp, axis=1))
    res = res - Z[index1, list(y)]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(iterations):
      ...
      Z[range(data_y.shape[0]), data_y] -= 1
      gradient = data_x.T @ Z
      gradient /= data_x.shape[0]
      theta -= lr * gradient
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(iterations):
      ...
      G2 = np.exp(output) / np.sum(np.exp(output), axis=1, keepdims=True)
      G2[range(data_y.shape[0]), data_y] -= 1
      G1 = G2 @ W2.T
      G1 = G1 * relu_mask
